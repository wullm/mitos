...
int main(int argc, char *argv[]) {
    ...
    /* Undo monofonic normalization (assuming monofonic says P(k) = 1) */
    const double Pk_norm = pow(2.0 * M_PI / cosmo.h, 3);
    const double norm = 1.0 / sqrt(Pk_norm);
    ...
    for (int x=0; x<N; x++) {
    	for (int y=0; y<N; y++) {
            for (int z=0; z<=N/2; z++) {
              fbox[row_major_half(x,y,z,N)][0] *= norm;
              fbox[row_major_half(x,y,z,N)][1] *= norm;
            }
        }
    }

...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        /* Number of subboxes on a side */
        int M = 4;

        double tot_dens = 0;

        /* Allocate the subboxes */
        int subN = N / M;
        double subBoxlen = boxlen[0] / M;
        double subAngleSize = anglesize / M;
        double *sub_box = malloc(subN * subN * sizeof(double));
        fftw_complex *sub_fbox = (fftw_complex*) fftw_malloc(subN*(subN/2+1)*sizeof(fftw_complex));
        ...
        printf("\n");
        printf("Doing %d boxes of size %d\n", M, subN);

        /* Create FFT plan for the subbox */
        fftw_plan sub_r2c = fftw_plan_dft_r2c_2d(subN, subN, sub_box, sub_fbox, FFTW_ESTIMATE);

        /* Allocate memory for the power spectra */
        int bins = 20;
        ...
        for (int i=0; i<M; i++) {
            for (int j=0; j<M; j++) {
                ...
                /* Accumulate the local density */
                double D = 0;

                /* Copy over the subbox */
                for (int x=0; x<subN; x++) {
                    for (int y=0; y<subN; y++) {
                        double d = box[(i*subN + x)*N + (j*subN + y)];
                ...
                /* Normalization */
                for (int x=0; x<subN; x++) {
                    for (int y=0; y<=subN/2; y++) {
                        sub_fbox[x*(subN/2+1)+y] *= subBoxlen*subBoxlen/(subN*subN);
                    }
                }

                /* Compute the power spectrum */
                calc_cross_powerspec_2d(subN, subAngleSize, sub_fbox, sub_fbox, bins, l_in_bins, power_in_bins, obs_in_bins);

                if (i==0 && j==0) {
                    printf("d ");
                    for (int u=0; u<bins; u++) {
                        printf("%f ", l_in_bins[u]);
                    }
                    printf("\n");
                    printf("%e ", 0);
                    for (int u=0; u<bins; u++) {
                        printf("%d ", obs_in_bins[u]);
                    }
                    printf("\n");
                }
                ...
                printf("%e ", D);
                for (int u=0; u<bins; u++) {
                    printf("%e ", l_in_bins[u] * (l_in_bins[u]+1)*power_in_bins[u]);
                }
                printf("\n");
            }
        }

...
int main(int argc, char *argv[]) {
    ...
    /* Open the X-coordinates dataset */
    hid_t h_halo_dat = H5Dopen2(h_halo_file, "Xc", H5P_DEFAULT);
    ...
    /* Allocate memory for the halo centre coordinates */
    ...
    /* Maximum number of halos per cell */
    int max_halos_per_cell = 100;

    /* Allocate grids */
    int *box = malloc(N * N * N * max_halos_per_cell * sizeof(int));
    int *halos = calloc(N * N * N, sizeof(int));
    ...
    for (int i=0; i<halo_num; i++) {
        ...
        /* Insert the halo id */
        box[id*max_halos_per_cell + halos[id]] = i;
        halos[id]++;
        if (halos[id] > max_halos_per_cell) {
    ...
    for (int k=rank; k<slabs+1; k+=MPI_Rank_Count) {
        ...
        for (int l=0; l<slab_size; l++) {
            ...
            for (int i=-1; i<=1; i++) {
                for (int j=-1; j<=1; j++) {
                    for (int k=-1; k<=1; k++) {
                        int halos_in_cell = halos[row_major(iX + i, iY + j, iZ + k, N)];

                        for (int h=0; h<halos_in_cell; h++) {

                            /* Halo id */
                            int h_id = box[row_major(iX + i, iY + j, iZ + k, N) * max_halos_per_cell + h];
                            ...
                            if (r2 < r2_max) {
                                // printf("%f (%f, %f) (%f, %f) (%f, %f) %d %d\n", r2, X, h_x, Y, h_y, Z, h_z, h, h_id);
                                /* Put the particle in a bin */
                                int bin = floor(r2 / r2_max * num_bins);
            ...
            /* Add to all the appropriate profiles */

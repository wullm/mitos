...
double perturbGrowthFactorAtLogTau(const struct perturb_spline *spline, double log_tau) {
    /* Indices in the tau directions */
    int tau_index = 0;
    /* Spacing (0 <= u <= 1) between subsequent indices */
    double u_tau;
    ...
    return (1 - u_tau) * spline->ptdat->D_growth[tau_index]
               + u_tau * spline->ptdat->D_growth[tau_index + 1];
}
...
double perturbLogGrowthRateAtLogTau(const struct perturb_spline *spline, double log_tau) {
    ...
    return (1 - u_tau) * spline->ptdat->f_growth[tau_index]
               + u_tau * spline->ptdat->f_growth[tau_index + 1];
}
...
double perturbHubbleAtLogTau(const struct perturb_spline *spline, double log_tau) {
    ...
    return (1 - u_tau) * spline->ptdat->H_Hubble[tau_index]
               + u_tau * spline->ptdat->H_Hubble[tau_index + 1];
}
